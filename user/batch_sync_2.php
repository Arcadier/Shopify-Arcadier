<?php
error_log('accessing external php file 2');
ini_set('max_execution_time', 0); // 0 = Unlimited
ini_set('memory_limit','1024000000');
init_set('max_input_time', 0);
...
$url =  $baseUrl . '/api/v2/plugins/'. $packageId .'/custom-tables/batch_data?pageSize=1&sort=-CreatedDateTime';
$batchDetails =  callAPI("POST",$admin_token, $url,  $batch_data);

$batch_no = $batchDetails['Records'][0]['batch_no'];
...
$authListById=$arc->searchTable($packageId, 'batch_sync_data', $data11);
$itemslist =  json_decode($authListById['Records'][0]['batch_data'], true);

bulk_sync_items_2($itemslist, $access_token, $shop,$baseUrl,$userId,$admin_token, $packageId,$arc, $is_shopify_code,$arcadier_categories, $category_map,$userEmail);

function bulk_sync_items_2($products, $access_token, $shop, $baseUrl, $userId, $admin_token, $packageId, $arc, $is_shopify_code,$arcadier_categories,$category_map,$userEmail) {
               ...
               // error_log(json_encode($images1));
                ...
                if ($has_variants) {
                    ...
                    foreach($variants as $variant){
                        ...
                        $id = $variant['id'];
                        $variant_image =  findItem1($images, $id);
    ...
    function findItem1(array $variants, int $id){
        foreach ($variants as $variant) {
