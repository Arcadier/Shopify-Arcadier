...
/*Auth Request*/
if(isset($content['auth']) && ($content['auth'] == 'auth')){
	if (isset($content['usr'])) {
     $cookie_domain = $_COOKIE["marketplace"]; 
     $cookie_path = "/user/plugins/".$pack_id."/"; 
    ...
    $m_domain = $arc->getMarketplaceDomain1();
    ...
    $merchant_arc_guid=$content['arc_user'];
    ...
    if($mag->initialise_magento_products($domain, $mag_token)){
    $data = [
        ...
        'domain' => $domain,
        'token' => $mag_token,
        //'arc_username' => $m_username,
        //'arc_password' => $m_password,
        'arc_domain' => $m_domain,
        'arc_token' => $arc_access_token,
        'auth_status' => 1
    ];
	$dataa = [
        'merchant_guid' => $merchant_arc_guid,
    ...
    $data_auth_status_deactivate = [
        'auth_status' => "0"
    ];
    $authListById=$arc->searchTable($pack_id, 'auth', $data_auth);
    foreach($authListById['Records'] as $authLists){
        $UpdateRowsInauth=$arc->editRowEntry($pack_id, 'auth', $authLists['Id'], $data_auth_status_deactivate);
    }
    ...
    if (!empty($mag_token)) {
        $checkRowInauthExist=$arc->searchTable($pack_id, 'auth', $data1);
        if(!empty($checkRowInauthExist['Records'])){
        $UpdateRowInauth=$arc->editRowEntry($pack_id, 'auth', $checkRowInauthExist['Records'][0]['Id'], $data2); 
        
        }else{
            $insertRowInauth=$arc->createRowEntry($pack_id, 'auth',$data);
			$insertRowInauth=$arc->createRowEntry($pack_id, 'config',$dataa);
        }
        
        /* if (!empty($_COOKIE["mag_user"])) {

            if (($_COOKIE["mag_user"]) == $username && ($_COOKIE["mag_pass"]) == $password && ($_COOKIE['mag_domain']) == $domain  && ($_COOKIE['m_username']) == $m_username   && ($_COOKIE['m_password']) == $m_password && ($_COOKIE['m_domain']) == $m_domain && ($_COOKIE['auth']) == 'auth') {
                ...
                setcookie("mag_user", $username, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("mag_pass", $password, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("mag_domain", $domain, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("mag_token", $mag_token, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("arc_access_token", $arc_access_token, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("merchant_arc_guid", $merchant_arc_guid, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("m_username", $m_username, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("m_password", $m_password, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("m_domain", $m_domain, time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
                setcookie("auth", "auth", time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        ...
        echo json_encode(array('message' => $message, 'row1' => $authListById['Records'][0]));
        ...
        setcookie("mag_user", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("mag_pass", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("mag_domain", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("mag_token", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("arc_access_token", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("merchant_arc_guid", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("m_username", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("m_password", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("m_domain", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain);
        setcookie("auth", '', time() + (10 * 365 * 24 * 60 * 60), $cookie_path,  $cookie_domain); */
...
if (!empty($content['arcadier_guid'])) {
    ...
    if ($response['Records'][0]['merchant_guid'] == $content['arc_user']) {
        ...
        foreach ($list as $key => $data) {
            if ($data['magento_cat'] == $magento_cat) {
                unset($list[$key]);
                ...
                $data2 = [
                    ...
                    'domain' => $authListByMerchantGuid['Records'][0]['domain']
                ];
...
/*plugin_license_config Custom Table Request*/
elseif(isset($content['plugin_license_config_req'])){
	/* $data1 = [
        [
          'Name'=> 'merchant_guid',
          'Operator'=> 'equal',
	...
	$data_i = [
        'stripe_publishable_key' => $content['s_p_k'],
        'stripe_secret_key' => $content['s_s_k'],
        'stripe_subscription_plan_id' => $content['s_s_p_id'],
        'subscription_amount' => $content['s_amount'],
        'total_trial_days' => $content['t_t_days']
    ];
    ...
    //$create_arc_item_slowListById=$arc->searchTable($pack_id, 'create_arc_item_slow', $data_create_arc_item_slow);
    $plugin_license_configListById=$arc->listTableData($pack_id, 'plugin_license_config');

    if(!empty($plugin_license_configListById['Records'])){
        $UpdateRowInplugin_license_config = $arc->editRowEntry($pack_id, 'plugin_license_config', $plugin_license_configListById['Records'][0]['Id'], $data_e);
        if(!empty($UpdateRowInplugin_license_config)){
            $message='Successfully updated data';
        }else{
            $message='Unable to update data';
        }
    }else{
        $insertRowInplugin_license_config = $arc->createRowEntry($pack_id, 'plugin_license_config', $data_i);
        if(!empty($insertRowInplugin_license_config)){
